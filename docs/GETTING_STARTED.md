...
to a local or configured LLM. It does not scrape paywalls: you get metadata plus
an abstract when there is no open-access copy.
...
topic. QC4HEP / Di Meglio 2024 is only the named `--preset qc4hep`.
...
Extraction projects live in `$CITEHOP_CORPORA_DIR/_projects/`.
...
(slug comes from the resolved title). Open `CATALOG.md` or the Corpus tab and
check titles, years, and that the seed is Vaswani 2017.
...
`--preset qc4hep` is the same command with a baked-in seed, not a different
code path.
...
`--yes` is required so a full neighborhood cannot start by accident. Re-run the
same command to resume a paused fetch. Full-text download is open-access only;
papers without an OA PDF stay as metadata + abstract.
...
Desktop: **Projects** → new project pointing at the corpus → **Schema** → apply
a template → edit types and fields.

CLI equivalent: create a project in the UI (project records live under
`_projects/`), or copy a template JSON to `schema.json` in that project
directory. `python -m citehop extract templates` lists template ids.
...
Select the model on **Models**, then **Extract** → Start (or Resume). Dual pass
is automatic: two generations per paper, then a merge.
...
Pause from the UI or `extract pause`. If the engine was still loading, or a
paper was skipped because the PDF had not arrived yet, **Resume** puts those
papers back in the queue. Unload the model when you are done so VRAM is free.
...
**Go to paper** opens the PDF or extracted text at the quoted span. Read the
quote against the paper. A `match` only means both passes agreed with each
other.
...
The file looks like this (truncated):
